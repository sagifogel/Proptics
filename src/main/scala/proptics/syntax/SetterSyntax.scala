...
object SetterSyntax {
  implicit class SetterSTAAOps[S, T, A](val setter: Setter_[S, T, A, A]) extends AnyVal {
  ...
  implicit class SetterSSABOps[S, A, B](val setter: Setter_[S, S, A, B]) extends AnyVal {
  ...
  implicit class SetterSAOps[S, A](val setter: Setter_[S, S, A, A]) extends AnyVal {
  ...
  implicit class SetterSTAOptionB[S, T, A, B](val setter: Setter_[S, T, A, Option[B]]) extends AnyVal {
    def setJust(b: B): S => T = setter.set(Some(b))
  }

  implicit class SetterSSAOptionB[F[_], S, A, B](val setter: Setter_[S, S, A, Option[B]]) extends AnyVal {
