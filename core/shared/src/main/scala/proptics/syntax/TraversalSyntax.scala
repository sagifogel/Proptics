...
trait TraversalSyntax {
  implicit def traversalElementOps[P[_, _], I, S, T, A](traversal: Traversal_[S, T, A, A]) = TraversalElementOps(traversal)

  implicit def traversalSequenceOps[F[_], S, T, A](traversal: Traversal_[S, T, F[A], A]) = TraversalSequenceOps(traversal)
}

final case class TraversalElementOps[P[_, _], I, S, T, A](private val traversal: Traversal_[S, T, A, A]) extends AnyVal {
