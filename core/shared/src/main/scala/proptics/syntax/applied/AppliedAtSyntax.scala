...
case class AppliedAtOfSetOps[A](private val set: Set[A]) extends AnyVal {
...
case class AppliedAtOfMapLikeOps[M[_, _], K, V](private val map: M[K, V]) extends AnyVal {
...
case class AppliedLensAtOps[S, T, A](private val appliedLens: AppliedLens[S, T]) extends AnyVal {
...
case class AppliedFoldAtOps[S, T, A](private val appliedFold: AppliedFold[S, T]) extends AnyVal {
  /** traverse a value at a given key of a data structure `S` */
  def at[I](i: I)(implicit ev: At[T, I, A]): AppliedFold[S, Option[A]] = appliedFold.andThen(ev.at(i))
}

case class AppliedAffineTraversalAtOps[S, T, A](private val appliedAffineTraversal: AppliedAffineTraversal[S, T]) extends AnyVal {
  /** traverse a value at a given key of a data structure `S` */
  def at[I](i: I)(implicit ev: At[T, I, A]): AppliedFold[S, Option[A]] = appliedAffineTraversal.andThen(ev.at(i))
}

case class AppliedTraversalAtOps[S, T, A](private val appliedTraversal: AppliedTraversal[S, T]) extends AnyVal {
  /** traverse a value at a given key of a data structure `S` */
